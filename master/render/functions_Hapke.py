...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr 22 22:05:00 2025

These functions compute the bidirectionnal reflectance of a monolayer for photometry using function from geometries and surface parameters using the Hapke formulation

reflectance_photometry_HG2


Inputs : 

    - singalb : single scattering albedo (also written as w in the function fct_reflectance_numba)

    - b : asymetry parameter of henyey greenstein phase function' 0<b<1, b=0. = isotropic (large lobe), =1 narrow lobe

    - c : backscattering fraction of the 2 term-henyey greenstein phase function  0<c<1, c=0 isotropic, c<0.5 forward scattering, c>0.5 backward scattering
    
    - dzeta : Hapke mean slope roughness parameter (theta bar) (°)
...
@author: 
Hugo Lancery <hugo.lancery@universite-paris-saclay.fr>
François Andrieu <francois.andrieu@universite-paris-saclay.fr>
Frédéric Schmidt <frederic.schmidt@universite-paris-saclay.fr>
...
def fct_reflectance_numba(w, b, c, dzeta, B0, h, incidence, emergence, phase):
    ...
    #print(mu-math.cos(math.pi*emergence/180.))
    ...
    #fct_reflectance=w/(4.*math.pi)*muo/(mu+muo)*((1.+B)*fct_phase_HG2(b,c,phase)+fct_h_numba(muo, gamma)*fct_h_numba(mu, gamma)-1)



    #REEF

    #fct_reflectance=w/(4.*math.pi)*muo/(mu+muo)*((1.+B)*fct_phase_HG2(b,c,phase)+fct_h_numba(muo, gamma)*fct_h_numba(mu, gamma)-1) * math.pi/math.cos(math.pi*incidence/180.)
