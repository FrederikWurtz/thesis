...
def train_epoch(model, train_loader, optimizer, scaler, device, train_mean, train_std, current_epoch=None, total_epochs=None, non_blocking=None,
    ...
    rank = dist.get_rank()
    print(f"[Rank {rank}] Starting train_epoch", flush=True)
    ...
    train_pbar = tqdm(train_loader, desc=f"Training on epoch {current_epoch}/{total_epochs}", leave=False, position=0, dynamic_ncols=True, disable = not is_main_process())
    ...
    # after loop:
    print(f"[Rank {rank}] Finished train_epoch", flush=True)
