...
from master.legacy_files.trainer_core_multi_gpu import DEMDataset
...
def main(run_dir: str):
    ...
    if torch.cuda.is_available() and torch.cuda.device_count() > 1:
        if is_main():
            print("Multiple GPUs and CUDA hardware detected - setting up multi-GPU testing...")
        use_multiGPU = True
        ddp_setup()
    else:
        use_multiGPU = False
    ...
    val_loader = prepare_dataloader(val_set, config["BATCH_SIZE"], 
                                    ...
                                    prefetch_factor=config["PREFETCH_FACTOR"],
                                    multi_gpu=use_multiGPU)
    if not args.test_on_separate_data: # normal testing
        test_loader = prepare_dataloader(test_set, config["BATCH_SIZE"], 
                                        num_workers=config["NUM_WORKERS_DATALOADER"], 
                                        prefetch_factor=config["PREFETCH_FACTOR"],
                                        multi_gpu=use_multiGPU)
    else: # testing on separate data
        alt_test_dir = os.path.join(run_path, 'alt_test')
